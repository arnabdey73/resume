...
Senior Cloud / DevOps Engineer with **15+ years in IT** and **9+ years focused on cloud and platform engineering** — deep hands-on AWS experience across EC2, EKS, S3, VPC, IAM, KMS, ALB, CloudWatch, built and operated as code with Terraform, Ansible, and Python. I design infrastructure for reliability, security, and developer self-service: version-controlled, reviewable, reproducible, and cheap to rebuild. Comfortable owning distributed microservice platforms end-to-end — provisioning, CI/CD, observability, on-call. Based in Stockholm, fluent in English.

---

## Core Skills

**AWS** — EC2, EKS, S3, VPC, IAM, ECR, ALB, CloudWatch, KMS; production platform design, least-privilege IAM, multi-environment delivery, cost optimisation

**Infrastructure as Code** — Terraform (AWS, Azure, OpenStack providers), reusable module libraries, Ansible; IaC patterns transferable to CloudFormation / AWS CDK (ramping alongside SAA-C03 preparation)

**Containers & Orchestration** — Kubernetes (EKS, Rancher RKE, K3s, AKS), Docker, Helm

**CI/CD** — GitHub Actions, Jenkins, GitLab CI, Azure DevOps Pipelines, ArgoCD; PR-driven `plan`/`apply` workflows, trunk-based delivery
...
**Networking & Linux** — cloud networking (VPC / subnet / security-group design, ALB, DNS, SSL/TLS), SAML federation, hybrid on-prem ↔ cloud connectivity, production Linux/Unix (RHEL/SLES/Ubuntu)

**Databases** — MySQL and MongoDB installation and configuration underpinning CI/CD platform estates; hands-on SQL querying via CLI and `.sql` scripts for maintenance and reporting

**Distributed Systems & Observability** — microservice platforms on Kubernetes, Prometheus, Grafana, ELK Stack (Elasticsearch, Logstash, Kibana), CloudWatch, Azure Monitor; dashboards, alerting design, log aggregation

**Security & IAM** — IAM least-privilege, custom RBAC role definitions, encryption at rest (KMS / Key Vault), secret management (sealed-secrets, Key Vault), credential rotation, separation-of-duties controls, compliance auditing

**On-call & Incident Response** — 24×7 shift-based operations background, blameless post-mortems, RCA-driven remediation
...
- **Backup policy compliance reporting** with per-VM exempt-rationale tracking; KQL-based monitoring contributed to a **~20% reduction in downtime** across supported services.
...
- Contributed to modernising a ~36-node CAE/HPC cluster supporting ML research: OS migration from SLES to Ubuntu LTS, stand-up of **Rancher-managed Kubernetes (RKE)**, **Kubeflow** via Helm.
- Built a **production-grade ELK stack** (Elasticsearch, Logstash, Kibana) on LXD with X-Pack security, SSL, and snapshot repositories — full-fleet observability and log aggregation.
...
- **Closed out Cisco's long-running SVN-to-Git migration** by writing a Python automation that enumerated SVN repos, extracted history, provisioned Bitbucket repos via REST API, and pushed converted Git history — retry logic and batching around SCM rate limits.
...
- Installed and operated the **MySQL and MongoDB instances** backing the CI/CD platform's supporting services; day-to-day SQL querying via CLI and `.sql` scripts for maintenance and reporting.

**Assignment: Deutsche Telekom — 5G BSS Slicing Pilot on OpenStack** *(Technical Lead, mid-2020 – Aug 2021)*
- Owned the **DevOps platform side** of DT's 5G Network Slicing commercial pilot — accountable for operational reliability of a **~30-microservice BSS suite** on Kubernetes (Rancher).
...
- Owned the **cross-tenant migration** when the pilot moved tenants: Python/shell automation to clone 54 Tuleap repos, re-seed Jenkins jobs, and rehome Nexus artifacts, Keycloak realm, and running Kubernetes workloads — cutover completed over a single weekend.
